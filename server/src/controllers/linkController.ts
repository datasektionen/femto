...
export async function deleteLink(req: Request, res: Response): Promise<void> {
    ...
    try {
        ...
        if (userPermissions.includes("manage-all")) {
            ...
            console.log(`🔑 User ${userId} has manage-all permission - deleting link ${slug} without checks`);
            const deleteResult = await client.query("DELETE FROM urls WHERE slug = $1 RETURNING slug", [slug]);
...
export async function updateLink(req: Request, res: Response): Promise<void> {
    ...
    try {
        ...
        if (userPermissions.includes("manage-all")) {
            ...
            console.log(`🔑 User ${userId} has manage-all permission - updating link ${slug}`);
            ...
            if (isOwner || hasGroupAccess) { // Updated condition
                ...
                if (updateResult.rowCount === 0) {
                    // This might happen if the link exists but doesn't match the final WHERE clause (should theoretically not happen due to prior checks, but good for safety)
                     console.warn(`🚫 Update for link ${slug} by user ${userId} failed unexpectedly after permission check.`);
                     res.status(404).send("Link not found or permission mismatch during update.");
                } else {
                    res.status(200).json(updateResult.rows[0]);
                }
            } else {
                console.warn(`🚫 User ${userId} denied update of link ${slug} - Not owner or matching mandate`);
        ...
        if (err.code === '23505') { // Unique violation error code in PostgreSQL
             res.status(409).json({ error: "Update failed due to conflicting data (e.g., unique constraint)." });
        } else {
            res.status(500).send("Internal Server Error");
        }
    } finally {
