...
const LinkStats: React.FC = () => {
    ...
    const [granularity, setGranularity] = useState<"day" | "hour" | "week">("day"); // Controls the time granularity for stats (day, hour, week)
    ...
    const form = useForm({
        ...
        validate: {
            ...
            url: (value) => ((value && /^(ftp|http|https):\/\/[^ "]+$/.test(value)) ? null : 'Ogiltig URL'),
        },
    });

    const {
        groups, // List of available groups for the user
    ...
    */
        ...
        api
            ...
            .then((res) => {
                ...
                if (granularity === "hour") {
                    ...
                    processedData = filteredForToday.length ? filteredForToday : getZeroLineDataForToday();
                    ...
                    const hitsLastWeek = processedData.filter((d) => {
                        ...
                        const dataDateUTC = new Date(
                            Date.UTC(dataDate.getUTCFullYear(), dataDate.getUTCMonth(), dataDate.getUTCDate())
                        );
                        return dataDateUTC >= startDateUTC && dataDateUTC <= endDateUTC;
                    });

                    // Fill missing days in the week with 0 clicks
                    const filledWeekData = fillMissingDays(hitsLastWeek, startDateUTC, endDateUTC);
    ...
    useEffect(() => {
        if (linkDetails) {
            form.setValues({
                ...
                // Convert UTC expiration date from API to local datetime string for the input field
                expires: linkDetails.expires ? toLocalISOString(linkDetails.expires) : '',
                group_name: linkDetails.group_name || '',
            });
        }
    }, [linkDetails, form.setValues]); // form.setValues is stable, but good practice to include if used
    ...
    const handleSaveChanges = async (values: typeof form.values) => {
        ...
        const payload = {
            ...
            // Convert 'expires':
            // - Empty string from form means null (no expiration).
            // - Otherwise, convert the local datetime string from form to a full ISO string (UTC) for backend.
            expires: values.expires ? new Date(values.expires).toISOString() : null,
            // Send null if group_name is empty, otherwise send the selected group name.
    ...
    const deleteLink = () => {
        ...
        if (window.confirm("Är du säker på att du vill ta bort länken? Denna åtgärd kan inte ångras.")) {
            api
    ...
    if (error && !linkDetails && !loadingDetails) { // Ensure not to show this if details just failed but were previously loaded
        return (
            <>
                <Header title="Fel" />
                <Box id="content">
                    <Alert color="red" title="Ett fel uppstod">{error}</Alert>
                    <Button mt="md" onClick={() => navigate("/links")} leftSection={<IconArrowLeft size={iconSize} />}>
                ...
                <Group grow align="flex-start">
                                ...
                                <form onSubmit={form.onSubmit(handleSaveChanges)}>
                                    ...
                                    {hasGroups && (
                                        <Select
                                            ...
                                            data={groups.map(g => ({ value: g, label: g }))} // Assuming groups is an array of strings
                                            searchable
                                            clearable
                                    ...
                                    {error && !loadingDetails && <Alert color="red" title="Fel vid sparning" my="md">{error}</Alert>}
                                    <Group mt="md">
                                        <Button type="submit" size="sm" variant="filled" radius="md">Spara ändringar</Button>
                                        <Button
                                            size="sm"
                                            variant="light"
                                            radius="md"
                    ...
                    <Box>
                        ...
                        <Card radius="lg" shadow="xs" withBorder p="md">
                            ...
                            {error && loadingStats && <Alert color="red" title="Fel vid hämtning av statistik">{error}</Alert>}
                            {loadingStats ? (
                                <Group justify="center" style={{ minHeight: 350 }}><Loader /></Group>
                            ) : (
                                <ResponsiveContainer width="100%" height={350}>
                                    ...
                                    >
                                        ...
                                        <XAxis
                                            dataKey="date"
                                            tickFormatter={(tickValue) => formatAxisDate(tickValue, granularity)}
                                            stroke="#adb5bd"
                                            tick={{ fontSize: 11 }}
                        ...
                        <Card radius="lg" shadow="xs" withBorder p="md" mt="lg">
                            <Title order={3} mb="sm">Klick per språk</Title>
                            {loadingLang && <Group justify="center" style={{ minHeight: 200 }}><Loader /></Group>}
                            {errorLang && <Alert color="red" title="Fel vid hämtning av språkstatistik">{errorLang}</Alert>}
